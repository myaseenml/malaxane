...
def create_generated_car_name(name):
    now = datetime.now()
    date_str = now.strftime("%Y-%m-%d")
    time_str = now.strftime("%H-%M-%S")
    return f"{name}_{date_str}_{time_str}"

if __name__ == "__main__":
    INPUT_CAR_PATH = input("Enter car image path: ")
    CAR_BG_PATH = input("Enter background image path: ")
    OUTPUT_CAR_PATH = create_generated_car_name("generate_car")
    
    image_PIL = Image.open(INPUT_CAR_PATH)
    hide_success, car_wo_plate_path = hide_plate(image_PIL)
    
    if hide_success:
        print("Car plate successfully hidden.")
        image_for_bg_removal = car_wo_plate_path
    else:
        print("No plate detected or failed to save processed image. Proceeding with the original image.")
        image_for_bg_removal = INPUT_CAR_PATH
    
    remove_car_bg(image_for_bg_removal)
    place_car_on_background(CAR_BG_PATH, OUTPUT_CAR_PATH)